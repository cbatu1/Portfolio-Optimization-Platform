...
import cvxopt as cvx  # This package may need to be installed
...
class PortOptimizer():
    ...
    # All these parameters need to be fed even though some of them may not be used
    ...
    def construct(self, method, covtype='sample'):
        ...
        if not (method in ['EqualWeight', 'InverseVol', 'RiskParity', 'MinVar', 'MaxDiver', 'MinCVaR']):
            ...
            if method == 'EqualWeight':
                ...
                wgt = qua_opti(robustcovest(self.df, covtype) * 10 ** 4, self.pena,
                               self.bound)  # In case that the covariance matrix is too small
